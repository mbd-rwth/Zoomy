...
from bokeh.palettes import Viridis256, Blues256, Category20b, Magma11
...
import game.swe.swe._2d.solver as sim
...
def update_image():
    ...
    tstart = get_time()
    Q = sim_step(Q)
    print(f'TIME FOR ONE STEP: {get_time()-tstart}')

image_source = ColumnDataSource(data=dict(image=[generate_image()]))
color_mapper = LinearColorMapper(palette=Magma11, low=0, high=255, nan_color='black')
...
pn.state.add_periodic_callback(update_image, period=5)  
