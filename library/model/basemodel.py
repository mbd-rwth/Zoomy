...
class Model:
    ...
    def __attrs_post_init__(self):
        ...
        object.__setattr__(
            ...
            self.boundary_conditions.get_boundary_condition_function(
                    self.time, self.position, self.distance, self.variables, self.aux_variables, self.parameters, self.normal),
    ...
    def source(self):
        return Arr.zeros(self.n_variables)
        return zeros(self.n_variables, 1)

    def quasilinear_matrix(self):
        """generated automatically unless explicitly provided"""
        return self._simplify(                    
                sympy.derive_by_array(self.flux(), self.variables.get_list()) + self.nonconservative_matrix(),
            )

    def source_jacobian_wrt_variables(self):
        """generated automatically unless explicitly provided"""
        return self._simplify(sympy.derive_by_array(self.source(), self.variables.get_list()))

    def source_jacobian_wrt_aux_variables(self):
        """generated automatically unless explicitly provided"""
        return self._simplify(sympy.derive_by_array(self.source(), self.aux_variables.get_list()))
    ...
    def left_eigenvectors(self):
        return Arr.zeros(self.n_variables, self.n_variables)
...
def substitute_precomputed_denominator(self, expr, sym, sym_inv):
    ...
    if isinstance(expr, sympy.Array):
        ...
        return sympy.Arr(new_data).reshape(*expr.shape)
