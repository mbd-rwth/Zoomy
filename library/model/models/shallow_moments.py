...
from library.model.models.basismatrices import Basismatrices, Legendre_shifted
...
class ShallowMoments(Model):
    ...
    def newtonian_boundary_layer(self):
        ...
        ha = self.variables[1 : 1 + self.levels + 1]
        p = self.parameters
        phi_0 = [self.basismatrices.basis.eval(i, 0.0) for i in range(self.levels + 1)]
        dphidx_0 = [
...
class ShallowMomentsSSFEnergy(ShallowMoments):
    def flux(self):
        flux = Matrix([0 for i in range(self.n_fields)])
        h = self.variables[0]
        ha = self.variables[1 : 1 + self.levels + 1] - self.variables[1] / np.diag(
...
class ShallowMoments2d(Model):
    ...
    def newtonian_boundary_layer(self):
        ...
        hb = self.variables[1 + offset : 1 + self.levels + 1 + offset]
        p = self.parameters
        phi_0 = [self.basismatrices.eval(i, 0.0) for i in range(self.levels + 1)]
        dphidx_0 = [
