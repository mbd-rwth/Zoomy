...
class Solver:
    ...
    def numerical_flux(self, model, Ql, Qr, Qauxl, Qauxr, parameters, n, domain):
        ...
        return ufl.dot(
            ...
            * (model.flux(Ql, Qauxl, parameters) + model.flux(Qr, Qauxr, parameters)),
            ...
            max_abs_eigenvalue(model, Ql, Qauxl, n, domain)
            + max_abs_eigenvalue(model, Qr, Qauxr, n, domain)
        ) * I * (Qr - Ql)
    ...
    def compute_time_step_size(
        self, model, Q, Qaux, n, reference_cell_diameter, domain, CFL=0.45
    ):
    ...
    def solve_time_loop(
        ...
        friction_coeff = dolfinx.fem.Constant(
            domain, dolfinx.default_scalar_type(self, 0.0)
        )
        ...
        weak_form_lhs, weak_form_rhs = weak_form_function(
            model,
            functionspace,
            q_n,
            q_np1,
        ...
        while t.value < end_time and i < max_timesteps:
            ...
            dt.value = compute_time_step_size(
                model, q_np1, qaux_np1, normals, reference_edge_length, domain, CFL=CFL
            )
