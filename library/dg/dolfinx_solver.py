...
class DolfinxHyperbolicSolver():
    ...
    def compute_time_step_size(self, model, Q, Qaux, n, reference_cell_diameter, domain):
    ...
    def weak_form(self, model, functionspace, q_n, q_np, qaux_n, qaux_np, parameters, t, x, dt, domain, cell_tags, facet_tags, unique_facet_tags, facet_boundary_function_id):
        # facet normals
    ...
    def solve(self, path_to_mesh, model):
        ...
        weak_form_lhs, weak_form_rhs = self.weak_form(model, functionspace, q_n, q_np1, qaux_n, qaux_np1, parameters, t, x, dt, domain, cell_tags, facet_tags, unique_facet_tags, facet_boundary_function_id)
        ...
        # VTK writer
        os.makedirs(os.path.join(main_dir, self.settings.output.directory), exist_ok=True)
        vtk_file_abs_path_name = os.path.join(os.path.join(main_dir, self.settings.output.directory), f"sim.pvd")
        ...
        progress = tqdm.tqdm(desc="Setup " + " sim " + ", solving PDE", total=self.time_end)
        ...
        while t.value < self.time_end and i < max_timesteps:
            ...
            dt.value  = self.compute_time_step_size(model, q_np1, qaux_np1, normals, reference_edge_length, domain)
...
class PreciceDolfinxHyperbolicSolver(DolfinxHyperbolicSolver):
    ...
    def weak_form(self, model, functionspace, q_n, q_np, qaux_n, qaux_np, q_precice, parameters, t, x, dt, domain, cell_tags, facet_tags, unique_facet_tags, facet_boundary_function_id):
        # facet normals
        # domain = functionspace.mesh
        n = ufl.FacetNormal(domain)
        ...
        for i, tag in enumerate(unique_facet_tags):
            ...
            if tag == self.precice_tag:
                weak_form += ufl.dot((test_q), self.numerical_flux(model, q, q_precice)) * ds(tag)
            else:
