...
line = create_line([0.,0.5], [1.,0.5], N)
...
def compute_z_integral(Q):
    h = Q.sub(0)
    u = vector_to_scalar(Q.sub(1), 0) / h
    v = vector_to_scalar(Q.sub(1), 1) / h
    phi = ( u.dx(0) + v.dx(1) ) / h
    h_line = assemble(interpolate(h, Vline))
    u_line = assemble(interpolate(u, Vline))
