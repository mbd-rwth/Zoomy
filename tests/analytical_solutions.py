...
def solution_ref_eq():
    ...
    eq = (simplify(diff(h*u, t))) + b*(simplify(diff(h*u*u, x))) + c*(simplify(diff(h*u*w, z)))
...
def sin_problem():
    ...
    eq = cos(a*x + b*t + c) * b + cos(a*x+b*t+c) * a * sin(al*x+be*t+ga) + sin(a*x+b*t+c) * al * cos(al*x+be*t+ga)
    print(eq)
    sol = solve(eq, al)
    print(sol)
...
def solve_constains_no_soace():
    ...
    a, b, c = symbols('a b c', real=True)
    nu, rho, C, g, ex, sliplength = symbols('nu rho C g ex sliplength', real=True, constant=True)
    ...
    sol_0 = sol[0]
    sol_substituted = {key: value.subs({nu: 1, sliplength: 1, C: 1, g: 1, ex: 1, h: 1}) for key, value in sol_0.items()}
...
def solution_slip():
    ...
    u_num = lambda z, a0, a1, a2: a0 * np.ones_like(z) + a1 * (1-2*z) + a2 * (1 -6*z + 6*z**2)
