...
def get_vectorized_space_operator(mesh, func: jax.lib.xla_extension.PjitFunction):
    I_validate = jnp.linspace(0, mesh.n_elements-1, mesh.n_elements, dtype=jnp.int32)[:int(mesh.n_elements / 2)]
    def space_operator(
        Q: jnp.ndarray) -> jnp.ndarray:
...
class SpaceOperator():
    ...
    def get_vectorized2_space_operator(self, mesh, func: jax.lib.xla_extension.PjitFunction):
