...
def test_channel_with_hole_2d():
    level = 2
    settings = Settings(name = "ShallowMoments2d", momentum_eqns = [1, 2] + [3+l  for l in range(2*level)], parameters = {'g':9.81, 'C': 16., 'nu': 0.0016}, reconstruction = recon.constant, num_flux = flux.LLF(), compute_dt = timestepping.adaptive(CFL=0.45), time_end = 5., output_snapshots = 100)
    ...
    inflow_dict = {i: 0. for i in range(0, 2*(1+level)+1)}
    inflow_dict[0] = 0.1
    inflow_dict[1] = -0.2
    outflow_dict = {}

    bcs = BC.BoundaryConditions(
        [BC.Wall(physical_tag='hole'), BC.Extrapolation(physical_tag='top'),BC.Extrapolation(physical_tag='bottom'), BC.InflowOutflow(physical_tag='left', prescribe_fields=inflow_dict), BC.InflowOutflow(physical_tag='right', prescribe_fields=outflow_dict)]
    )
    ic = IC.Constant(constants=lambda n_fields:np.array([1.0, 0.1] + [0. for i in range(n_fields-2)]))
    model = ShallowMoments2d(
        dimension=2,
        fields=3+2*level,
        aux_fields=0,
    ...
    main_dir = os.getenv("SMS")
    mesh = Mesh.load_gmsh(
        os.path.join(main_dir, "meshes/channel_2d_hole/mesh_mid.msh"),
        'triangle'
    )
...
def test_smm_grad_2d():
    mesh_type='triangle'
    level = 2
    settings = Settings(name = "ShallowMoments2d", momentum_eqns = [1, 2] + [3+l  for l in range(2*level)], parameters = {'g':1.0, 'C': 16., 'nu': 0.1}, reconstruction = recon.constant, num_flux = flux.LLF(), compute_dt = timestepping.adaptive(CFL=0.45), time_end = 1., output_snapshots = 30)
    ...
    fvm_unsteady_semidiscrete(mesh, model, settings, RK1)
    # io.generate_vtk(settings.output_dir)
    postprocessing.recover_3d_from_smm_as_vtk(model, settings.output_dir, os.path.join(settings.output_dir, 'mesh.hdf5'), os.path.join(settings.output_dir, 'fields.hdf5'),Nz=10, start_at_time=1. )
