...
def test_smm_1d(level = 0, process='', case='', c_nut=1.0, c_bl=1.0, c_slipmod=1.0, lamda=7., nut = 0.0000145934315, nut_bl =0.0000145934315):
    ...
    settings = Settings(
        name="ShallowMoments",
        parameters={"g": 9.81,"nu": 0.000001, "rho":1.000, "lamda": lamda, "C":30., "kst":100, "eta":1., "c_nut":c_nut, "c_bl":c_bl, "c_slipmod":c_slipmod, "nut": nut, "nut_bl": nut_bl},
    ...
    bcs = BC.BoundaryConditions(
        [
            ...
            BC.Wall(physical_tag="right", momentum_field_indices=[[i] for i in range(1, level+1)])
        ]
    )
    ic = IC.RP(
    ...
    model = ShallowMoments(
        fields=2 + level, aux_fields=0,
        parameters=settings.parameters,
        boundary_conditions=bcs,
        initial_conditions=ic,

        #settings={"eigenvalue_mode": "symbolic", "friction": ['newtonian', "slip_mod", "newtonian_turbulent"]},
        settings={"eigenvalue_mode": "symbolic", "friction": ['newtonian', "newtonian_turbulent", "slip_mod",  "newtonian_boundary_layer_classic"]},
